...
- **TIMER_LEAKS — UPSTREAM PR READY 2026-04-22.** Branch `claude/timer-leaks-pattern` (off `main`) adds `docs/implementations/TIMER_LEAKS.md` (5 variants + 7 Key Lessons, `badge: Convention`, `order: 11`) and a new `### Timer and Subscription Cleanup` subsection in `CLAUDE.md §Code Standards` (placed between `### Cleanup` and `### Quality Checks`) so every downstream CLAUDE.md inherits the rule at the preferences level. Once merged to `main`, synctone's local `TIMER_LEAKS.md` can be deleted with the rest of the folder as part of its CLAUDE.md alignment chunk.
