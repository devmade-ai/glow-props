...
## Fleet sync — open decisions

The session-behaviour rewrite and the `align` fix pass have both been applied to
all 18 convention-set repos. What is left is one decision that is not a session's
to make:

- [ ] **Decide where `hf-sculpt` belongs.** It runs the fleet conventions, but it has no `public/projects/` entry, so `audit:discoverability` and `audit:cross-links` cannot grade it — they read their origins from the portfolio. Either give it a portfolio entry (and it joins both audits) or add it to the audit-exclusion list beside `sp-website` and `sp-backend`. Record the reason either way. Same question, unanswered, is what left it in no matrix and no exclusion list until 2026-08-15.
