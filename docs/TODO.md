...
- [ ] **glow-props TIMER_LEAKS self-compliance audit** — Pattern was upstreamed 2026-04-22 (`docs/implementations/TIMER_LEAKS.md` + `### Timer and Subscription Cleanup` subsection in CLAUDE.md Code Standards). Eat own dog food: audit `src/pwa.js:31,73,77,262` and `src/markdown.js:107,119` (raw `setTimeout`/`setInterval` calls without visible cleanup). For each: confirm cleanup handle exists in scope, or add one per the pattern's nested-timeout-array / AbortController / per-effect-dispose / HMR-guard variants.
...
Fully clean (no pending items): **canva-grid, budgy-ting, repo-tor, sun-sea-o, graphiki, synctone** (6 repos).
Fully clean on core patterns, trailing items only: **glow-props** (TIMER_LEAKS self-audit), **few-lap** (DOWNLOAD_PDF decision + __DEV__ guard restore).
...
17. **NEW: TIMER_LEAKS self-compliance** — Pattern was upstreamed to glow-props 2026-04-22. glow-props itself has unaudited raw timers (`src/pwa.js:31,73,77,262`, `src/markdown.js:107,119`). Downstream repos should inherit `### Timer and Subscription Cleanup` subsection in their CLAUDE.md Code Standards — confirmed missing in model-pear, likely missing elsewhere.
