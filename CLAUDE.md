...
- **DISCOVERABILITY in this repo:** glow-props follows the PUBLIC column of its own [`docs/implementations/DISCOVERABILITY.md`](docs/implementations/DISCOVERABILITY.md) — indexable, `robots.txt` allowing the crawl and naming the sitemap, per-page canonical, full Open Graph + Twitter coverage, and a 1200×630 card generated from the icon (`npm run generate:og-image`). Four things are deliberate and easy to undo by accident: (1) **every pattern AND every project is prerendered** — `prerenderPatternPages()` and `prerenderProjectPages()` in `vite.config.js` each emit one real file per item (`patterns/<slug>/index.html`, `projects/<slug>/index.html`), derived from the BUILT template page, with its own head tags and its markdown rendered into `#app` (which the page's own render then overwrites, so there is no shell to tear down). The clean URLs are canonical; the `?name=` forms still work and point at them. Both prerenderers also rebase document-relative assets (`theme.js`, icon links, navbar mark) via `rebaseDocumentRelativeAssets()` — nested pages 404 on relative paths without it. (2) `sitemap.xml` is GENERATED at build from `docs/implementations/` and `public/projects/` — adding `public/sitemap.xml` would shadow it. (3) `pattern.html` / `project.html` carry NO static canonical, because their content is chosen by `?name=` and a fixed one would collapse every item onto a single URL; it is set at runtime in `src/seoMeta.js`. (4) the landing page's pattern cards are injected statically at build (crawlers that don't run JS discover the pattern pages through them; the runtime render replaces them). `npm run verify:seo` is the tripwire for all of it — **run `npm run build` first**, since its checks over `dist/` are only as current as the last build.
- **PWA_ICON_CACHE_BUST in this repo:** fully implemented in `vite.config.js` — `iconVersion()` (sha256, 8 hex) + `versioned()` feed the manifest `icons[]`, and `iconCacheBustHtml()` rewrites the icon links + navbar mark in every page (fail-loud on a reformatted tag). Workbox `ignoreURLParametersMatching` includes `/^v$/` and `/^name$/` (offline lookup for versioned icons AND `?name=` pages), and `globIgnores` excludes the icon files so each has exactly ONE precache entry — a bare-URL duplicate alongside a revisioned one makes workbox-precaching throw `add-to-cache-list-conflicting-entries` and kills the whole SW. The manifest's maskable icon is the separate full-bleed `icon-1024-maskable.png` (generated by flattening the transparent source onto white in `scripts/generate-icons.mjs`); the favicon/navbar set stays transparent on purpose. `npm run verify:icons` is the tripwire (dist checks need a build first) and gates the deploy alongside `verify:seo`.
- **TIMER_LEAKS canonical examples in this repo:** `src/pwa.js` demonstrates variants 1 (nested timeouts in `showToast`), 4 (module dispose via `import.meta.hot.dispose`), and 5 (`window.__pwaModuleAttached` HMR guard for global listeners). `src/markdown.js` demonstrates variant 4. `public/theme.js` demonstrates variant 4 for a static-asset script (no HMR; `window.__theme.dispose()` exposed for tests/manual re-init). Inline page scripts use AbortController + a `window.__<page>` dispose (`window.__home` in `index.html`, `window.__project` in `project.html`) or an attach guard (`window.__pwaInstallCaptureAttached` in `partials/head-common.html`, released by `src/pwa.js`'s dispose). New listener/timer registrations in this repo should match one of these shapes; `npm run verify:timer-cleanup` is a static tripwire covering `src/` modules, every `public/*.js`, and inline `<script>` blocks in the three pages + partials.
- **Not Applicable Patterns (this repo):**
  - **EVENT_BUS — N/A.** Static four-module site with no service or data layer. The only pub/sub-shaped needs are cross-tab theme sync (`public/theme.js` `storage` listener — the route EVENT_BUS.md itself prescribes for cross-tab) and the one-shot `beforeinstallprompt` handoff (`partials/head-common.html` → `src/pwa.js`, a one-off callback per EVENT_BUS.md's exclusions). `window.__pwa`/`window.md`/`window.__theme` are imperative command APIs, not notification channels. Revisit if a module ever needs to notify an unknown set of consumers.
...
- **BURGER_MENU deviations (this repo, deliberate):** (1) **No backdrop overlay** — the navbar's `backdrop-blur-md` creates a containing block that traps `position:fixed` children, so click-outside uses a document-level handler instead (rationale in `public/theme.js`; `Z_INDEX_SCALE.md` documents the same trap). Known cost: iOS Safari may not deliver whitespace taps to document-level click handlers. (2) **Body scroll lock** — `openMenu()` sets `overflow: hidden` + `scrollbar-gutter: stable` despite the pattern preferring `overscroll-contain` alone, because taps on non-scrollable menu areas still chain to the body without it; single writer, restored in `closeMenu()` and `dispose()`.
