...
- **DISCOVERABILITY in this repo:** glow-props follows the PUBLIC column of its own [`docs/implementations/DISCOVERABILITY.md`](docs/implementations/DISCOVERABILITY.md) — indexable, `robots.txt` allowing the crawl and naming the sitemap, per-page canonical, full Open Graph + Twitter coverage, and a 1200×630 card generated from the icon (`npm run generate:og-image`). Three things are deliberate and easy to undo by accident: (1) **every pattern is prerendered** to `patterns/<slug>/index.html` by `prerenderPatternPages()` in `vite.config.js` — one real file per pattern, derived from the BUILT `pattern.html`, with its own head tags and its markdown rendered into `#app` (which the page's own render then overwrites, so there is no shell to tear down). The clean URL is canonical; `pattern.html?name=` still works and points at it. (2) `sitemap.xml` is GENERATED at build from `docs/implementations/` and `public/projects/` — adding `public/sitemap.xml` would shadow it. (3) `pattern.html` / `project.html` carry NO static canonical, because their content is chosen by `?name=` and a fixed one would collapse every item onto a single URL; it is set at runtime in `src/seoMeta.js`. `npm run verify:seo` is the tripwire for all of it — **run `npm run build` first**, since its checks over `dist/` are only as current as the last build.
